...
public class RetireService {
	...
	private ResponseEntity<AccountModel> findAccountByNumberAccount(String number){
		return Consumer.webclientAccount.get().uri("/".concat(number)).retrieve().onStatus(status -> status.value() == 404, clientResponse -> Mono.empty()).toEntity(AccountModel.class)
	            .block();
	}
